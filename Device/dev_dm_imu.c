...
Result = (x_float - x_min) / (x_max - x_min) * (1 << bits - 1) -- 先是线性映射，然后乘上16位uint的范围。
************************************************************************
**/
...
void IMU_UpdateEuler(uint8_t* pData)
{
	...
	imu.pit=uint_to_float(euler[0],PITCH_CAN_MIN,PITCH_CAN_MAX,16);
	imu.yaw=uint_to_float(euler[1],YAW_CAN_MIN,YAW_CAN_MAX,16);
	imu.rol=uint_to_float(euler[2],ROLL_CAN_MIN,ROLL_CAN_MAX,16);
}
